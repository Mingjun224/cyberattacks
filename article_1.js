...
const articleHeader = document.querySelector('.article_1.header');
let headerIndex = 0;
const articleHeadline = document.querySelector('.article_1.headline');
let headlineIndex = 0;
const articleBodyone = document.querySelector('.article_1.body.one');
let bodyoneIndex = 0;
const articleBodytwo = document.querySelector('.article_1.body.two');
let bodytwoIndex = 0;
const articleBodythree = document.querySelector('.article_1.body.three');
...
document.addEventListener("keydown", (event) => {
  if (event.key === "ArrowRight" && event.ctrlKey) {
    articleHeader.textContent = headeroriginalText.substring(0, headerIndex + 102);
    articleHeadline.textContent = headlineoriginalText.substring(0, headlineIndex + 127);
    articleBodyone.textContent = bodyoneoriginalText.substring(0, bodyoneIndex + 381);
    articleBodytwo.textContent = bodytwooriginalText.substring(0, bodytwoIndex + 157);
    articleBodythree.textContent = bodythreeoriginalText.substring(0, bodythreeIndex + 256);
      }
    });
