window.onload = function() {
    ...
    const articleHeader = document.querySelector('.article_6.header');
    let headerIndex = 0;
    const articleHeadline = document.querySelector('.article_6.headline');
    let headlineIndex = 0;
    const articleBodyone = document.querySelector('.article_6.body.one');
    let bodyoneIndex = 0;
    const articleBodytwo = document.querySelector('.article_6.body.two');
    let bodytwoIndex = 0;
    const articleBodythree = document.querySelector('.article_6.body.three');
...
image.style.display = 'none';
    ...
    document.addEventListener('keypress', (event) => {
      ...
      if (headerIndex > 74 && /^[a-zA-Z]$/.test(key)){
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "ArrowRight" && event.ctrlKey) {
        articleHeader.textContent = headeroriginalText.substring(0, headerIndex + 74);
        articleHeadline.textContent = headlineoriginalText.substring(0, headlineIndex + 207);
        articleBodyone.textContent = bodyoneoriginalText.substring(0, bodyoneIndex + 372);
        articleBodytwo.textContent = bodytwooriginalText.substring(0, bodytwoIndex + 290);
        articleBodythree.textContent = bodythreeoriginalText.substring(0, bodythreeIndex + 465);
      }
    });
