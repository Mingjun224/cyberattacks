window.onload = function() {
    ...
    const articleHeader = document.querySelector('.article_8.header');
    let headerIndex = 0;
    const articleHeadline = document.querySelector('.article_8.headline');
    let headlineIndex = 0;
    const articleBodyone = document.querySelector('.article_8.body.one');
    let bodyoneIndex = 0;
    const articleBodytwo = document.querySelector('.article_8.body.two');
    let bodytwoIndex = 0;
    const articleBodythree = document.querySelector('.article_8.body.three');
...
image.style.display = 'none';
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "ArrowRight" && event.ctrlKey) {
        articleHeader.textContent = headeroriginalText.substring(0, headerIndex + 72);
        articleHeadline.textContent = headlineoriginalText.substring(0, headlineIndex + 166);
        articleBodyone.textContent = bodyoneoriginalText.substring(0, bodyoneIndex + 171);
        articleBodytwo.textContent = bodytwooriginalText.substring(0, bodytwoIndex + 243);
        articleBodythree.textContent = bodythreeoriginalText.substring(0, bodythreeIndex + 193);
      }
    });
