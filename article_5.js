window.onload = function() {
    ...
    const articleHeader = document.querySelector('.article_5.header');
    let headerIndex = 0;
    const articleHeadline = document.querySelector('.article_5.headline');
    let headlineIndex = 0;
    const articleBodyone = document.querySelector('.article_5.body.one');
    let bodyoneIndex = 0;
    const articleBodytwo = document.querySelector('.article_5.body.two');
    let bodytwoIndex = 0;
    const articleBodythree = document.querySelector('.article_5.body.three');
...
image.style.display = 'none';
    ...
    document.addEventListener('keypress', (event) => {
      ...
      if (headerIndex > 71 && /^[a-zA-Z]$/.test(key)){
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "ArrowRight" && event.ctrlKey) {
        articleHeader.textContent = headeroriginalText.substring(0, headerIndex + 71);
        articleHeadline.textContent = headlineoriginalText.substring(0, headlineIndex + 132);
        articleBodyone.textContent = bodyoneoriginalText.substring(0, bodyoneIndex + 315);
        articleBodytwo.textContent = bodytwooriginalText.substring(0, bodytwoIndex + 428);
        articleBodythree.textContent = bodythreeoriginalText.substring(0, bodythreeIndex + 108);
      }
    });
